...
class YOLODataset(Dataset):
    def __init__(
        ...
        self.all_image_list = config.IMAGE_SIZE
    ...
    def get_transform(self):
        if self.category =="train":
            self.transform = A.Compose(
                [
                    A.LongestMaxSize(max_size=int(self.image_size * config.scale)),
                    A.PadIfNeeded(
                        min_height=int(self.image_size * config.scale),
                        min_width=int(self.image_size * config.scale),
    ...
    def __getitem__(self, index):
        ...
        if prob<=0.6:
            self.image_size = self.all_image_list[0]
            
        elif prob<=0.8:
            self.image_size = self.all_image_list[1]
        else:
            self.image_size = self.all_image_list[2]
